...
- Write in flowing analyst prose: complete sentences and short paragraphs, the way a Korean sell-side analyst writes a note — not terse fragment bullets. Use bullet lists ONLY for discrete metric enumerations (headline vs consensus, KPI figures). No Markdown tables.
- Keep information density high: preserve the specific drivers, segment/region detail, and figures a professional analyst would want; do not thin the content just to sound smooth.
...
문체: 개조식 단어 나열이 아니라, 완결된 문장과 짧은 문단으로 이어지는 서술체로 쓴다. 한국 애널리스트가 리포트에 쓰는 문장처럼 자연스럽게. 다만 '핵심 실적 vs 컨센서스'와 '특이사항 & 핵심 KPI'의 수치 항목은 bullet로 나열해도 된다.
정보 밀도는 유지한다: 구체적 동인·세그먼트/지역별 수치·핵심 근거를 서술 안에 담되, 매끄럽게 쓰려고 내용을 덜어내지 않는다.
모바일 가독성을 위해 문단은 짧게 끊는다. 중복 숫자와 장황한 설명은 줄인다.
각 섹션 제목과 Q&A 질문 제목은 Telegram HTML bold 태그로 감싼다. 예: <b>📊 핵심 실적 vs 컨센서스</b>, <b>Q1. 북미 수요 회복</b>
Markdown table은 절대 사용하지 않는다.
...
<b>💬 경영진 핵심 메시지</b>
한 문단(3~5문장)의 서술체로, 전체 실적 흐름, 강했던·약했던 사업부·지역·제품, 수요·가격·물량·믹스·마진, 다음 분기 또는 하반기 핵심 변수를 자연스럽게 엮어 쓴다.

<b>⚡ 서프라이즈 포인트</b>
2~3개를 각각 완결된 서술 문장으로 쓴다(단순 "제목: 설명" 나열이 아니라 왜 서프라이즈인지 드러나게). 비교 기준이 없으면 surprise라고 쓰지 마라.

<b>🧾 특이사항 & 핵심 KPI</b>
중요 수치 3~5개를 bullet로 나열한다. 각 항목은 기간·단위를 함께 쓰고 완결된 구로 쓴다.

<b>🧭 가이던스 및 변경사항</b>
transcript에 실질적인 guidance, outlook, tone change가 있을 때만 이 섹션을 출력하고, 없으면 전체를 생략한다.
의미 있는 항목만 짧은 서술 문단으로 쓰되(미제시 항목을 억지로 채우지 않는다), 연간·분기 핵심 수치(EPS·매출·마진·FCF 등)는 문장 안에 명시한다.
각 항목의 변경 성격은 다음 중 하나로 규정한다: 상향 / 하향 / 재확인 / 범위 축소 / 범위 확대 / 철회 / 정성적 변경 / 확인 불가.
필요하면 기존 대비 달라진 점·주요 전제·압박/상쇄 요인을 짧게 덧붙인다.
