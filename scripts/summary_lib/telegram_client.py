...
def build_telegram_messages(
    ...
    *,
    ...
    for result in selected:
        ...
        # 발표일을 제목에 항상 표기. 파일명 '..._Earnings Call_YYYY-MM-DD_언어.docx'
        # 에서 추출하므로 LLM 출력과 무관하게 확정적이다.
        date_match = re.search(r"_(\d{4}-\d{2}-\d{2})_", result.get("file_name", ""))
        if date_match and date_match.group(1) not in title:
            title = f"{title} | {date_match.group(1)} 발표"
        header = f"<b>{title}</b>"
