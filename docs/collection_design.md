...
- 세션 만료 시 `CAPIQ_EMAIL`, `CAPIQ_PASSWORD`가 있으면 이메일·비밀번호를 자동 입력한다.
- MFA 4자리 코드는 자동 조회하지 않는다. 인증 메일이 Gmail이 아니므로 IMAP 자동조회
  로직을 제거했고, 세션 만료 시 열린 브라우저 창에서 사용자가 직접 MFA를 입력한다.
- 무인 실행 중 만료되면 최대 5분 대기 후 exit 2로 종료하고, 통합 파이프라인이 알림을 보낸다.
- 자격증명과 MFA 코드는 설정 파일·로그에 저장하지 않는다.
